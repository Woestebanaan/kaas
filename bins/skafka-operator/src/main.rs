...
use kube::{Api, Client};
use sk_controller::{KubeLeaseElection, LeaseElection};
use sk_operator_api::{KafkaCluster, KafkaTopic, KafkaUser};
use sk_operator_controllers::{
...
const LEADER_LEASE: &str = "skafka-operator-leader";
...
async fn main() -> Result<()> {
    ...
    let identity = std::env::var("HOSTNAME").unwrap_or_else(|_| "skafka-operator-0".into());
...
async fn spawn_metrics_server(addr: String, cancel: CancellationToken) {
    let app = Router::new().route(
        ...
        get(|| async { (StatusCode::OK, "# skafka-operator metrics — placeholder\n") }),
    );
    let listener = match tokio::net::TcpListener::bind(&addr).await {
        Ok(l) => l,
        Err(e) => {
    ...
    S: futures::Stream<
            Item = std::result::Result<
                ...
                kube::runtime::controller::Error<
                    sk_operator_controllers::ControllerError,
