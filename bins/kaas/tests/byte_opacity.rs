...
//!    [`kaas_codec::tripwires`] process-atomics stay at their reset
//!    baseline. If a future refactor decodes a record or re-encodes
//!    a batch, the corresponding
//!    `kaas_codec::tripwires::bump_codec_*` call fires and this test
//!    turns red.
//!
...
/// `crates/kaas-storage/src/memory.rs:80-126`.
...
async fn storage_round_trip_is_byte_identical() {
    ...
    assert_eq!(
        ...
        0,
        "byte-opacity violated: kaas_codec::tripwires::record_decode_count > 0. \
         Some code path called bump_codec_record_decode — see the tracing::warn line \
         from kaas_observability::byteopacity for the site."
    );
    assert_eq!(
        tripwires::batch_reencode_count(),
        0,
        "byte-opacity violated: kaas_codec::tripwires::batch_reencode_count > 0. \
