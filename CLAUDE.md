...
**`/readyz` is a one-shot latch, not a serving signal (gh #208).** `bins/kaas/src/main.rs` calls `set_ready(true)` as soon as the listeners bind — before any partition takeover — and nothing ever flips it back. So a broker reports Ready ~20 ms into boot and the StatefulSet rolling update moves straight to the next pod while this one is still grinding through takeover. Don't "just" gate readiness on takeover completion: `alive_brokers()` in `bins/kaas/src/cluster.rs` filters on EndpointSlice readiness, so a NotReady broker is invisible to the balancer, gets assigned zero partitions, has nothing to take over, and reports Ready anyway. Breaking that circle needs the alive set decoupled from pod readiness (Strimzi's `KafkaRoller` avoids it by asking the Admin API out-of-band rather than reading readiness at all). Note the PDB is inert for rollouts — StatefulSet updates delete pods directly rather than via the eviction API — though it still applies to node drains.
