...
- `crates/kaas-k8s` — broker-side K8s helpers: `endpoints.rs` (watches the headless service for peer endpoints), `identity.rs` (parses the ordinal out of the StatefulSet pod name), `topic_watcher.rs` (pure-state `KafkaTopic` cache with deletionTimestamp-immediate delete events and `topic_id` stashing — currently **un-wired**; production uses `kube_watchers::run_topic_watch`, see "KafkaTopic delete on NFS"), `readiness.rs` (satisfies the `kaas.rs/PartitionsReady` readiness gate once partition directories have been created on the PVC).
