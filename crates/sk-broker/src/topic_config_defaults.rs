...
//! the existing `sk_storage::TopicConfigFile` reader is the
...
//! discriminant (see [`sk_codec::api::describe_configs::config_type`]).

use sk_codec::api::describe_configs::config_type;
...
/// honours (gh #116 cleaner + compactor + retention). The Go
/// reference exposes the same set; clients gate on these for
