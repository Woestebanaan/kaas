...
//! satisfied by `sk-broker::Coordinator` in production (gh #92) and
...
//! wire-level error code so the handler layer in `sk-broker` only has
...
/// "Do I coordinate group G?" — answered by `sk-broker::Coordinator`
...
/// `sk-broker::Coordinator` into both seams; Phase 6 brings up the
...
/// real `sk-broker::Coordinator` is up; `cluster_runtime` (Phase 5
...
impl Manager {
    ...
    /// from `bins/skafka/main.rs` after `sk-broker::Coordinator`
...
/// shape in `sk-codec::api::find_coordinator`. The handler picks
