//! Per-connection framing wrapper around [`sk_codec::frame`].
//!
...
use sk_codec::frame::{write_frame, FrameError, FrameReader, MAX_FRAME_SIZE};
use sk_codec::headers::{decode_request_header, encode_response_header, HeaderVersion};
use sk_codec::{CodecError, RequestHeader, ResponseHeader};
use thiserror::Error;
...
mod tests {
    ...
    use sk_codec::primitives::write_compact_string;
    use sk_codec::tagged;
    ...
    async fn read_request_parses_header_via_registry() {
        ...
        tokio::spawn(async move {
            sk_codec::frame::write_frame(&mut a, &body).await.unwrap();
        });
    ...
    async fn write_response_emits_correlation_id_and_optional_tags() {
        ...
        let frame = sk_codec::frame::read_frame(&mut b).await.unwrap();
    ...
    async fn short_frame_rejected() {
        ...
        tokio::spawn(async move {
            ...
            sk_codec::frame::write_frame(&mut a, &[0u8; 2])
                .await
                .unwrap();
        });
        let mut conn = Connection::new(b);
