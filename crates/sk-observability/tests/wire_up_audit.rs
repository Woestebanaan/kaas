...
/// (see `crates/sk-observability/src/metrics.rs`) for eyeball diff.
...
///   `bins/skafka/main::cluster` and hasn't been threaded through
///   yet.
...
fn workspace_root() -> PathBuf {
    ...
    // sk-observability -> crates -> workspace root.
    p.pop();
    p.pop();
    p
}
...
fn walk_rs_files(root: &Path, out: &mut Vec<PathBuf>) {
    ...
    for entry in entries.flatten() {
        ...
        if ty.is_dir() {
            // Skip the sk-observability tree so the audit doesn't
            // catch the field-declaration lines in `metrics.rs`.
            if path.ends_with("sk-observability") || path.ends_with("target") {
                continue;
            }
