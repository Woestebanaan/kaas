...
//!   broker's cleaner / compactor via `sk_storage::TopicConfigFile`.
...
use kube::{Api, Client};
use sk_operator_api::{Condition, KafkaTopic};
use sk_storage::TopicConfigFile;
...
impl KafkaTopicReconciler {
    ...
    pub async fn reconcile(&self, topic: Arc<KafkaTopic>) -> Result<Action, ControllerError> {
        ...
        sk_storage::write_topic_config(&fs, &topic_dir, &cfg)?;
    ...
    async fn patch_status(
        &self,
        topic: &KafkaTopic,
        mutate: impl FnOnce(&mut sk_operator_api::KafkaTopicStatus),
...
/// Bonus helper for the `bins/skafka-operator` main: derive the
...
mod tests {
    ...
    use sk_operator_api::{KafkaTopic, KafkaTopicConfig, KafkaTopicSpec, KafkaTopicStatus};
