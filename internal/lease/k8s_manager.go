...
// ---- Coordinator lease methods ----

// AcquireCoordinator starts a LeaderElector for the given consumer group.
// Lease name: "skafka-coord-{sanitized-groupID}".
func (m *KubernetesLeaseManager) AcquireCoordinator(ctx context.Context, groupID string) error {
	...
	// Use partKey so IsLeader/LeaderFor (called by IsCoordinator/CoordinatorFor) find the right key.
	pkey := partKey("coord/"+groupID, 0)

	lock := &resourcelock.LeaseLock{
		LeaseMeta: metav1.ObjectMeta{
			Name:      coordLeaseName(groupID),
			Namespace: m.namespace,
		},
		Client:     m.client.CoordinationV1(),
...
func (m *KubernetesLeaseManager) ReleaseCoordinator(groupID string) error {
	return m.Release("coord/"+groupID, 0)
}

// IsCoordinator reports whether this broker holds the coordinator lease for groupID.
func (m *KubernetesLeaseManager) IsCoordinator(groupID string) bool {
	return m.IsLeader("coord/"+groupID, 0)
}
...
func (m *KubernetesLeaseManager) CoordinatorFor(groupID string) int32 {
	return m.LeaderFor("coord/"+groupID, 0)
}

// WaitForCoordinator polls until any broker holds the coordinator lease or ctx is cancelled.
func (m *KubernetesLeaseManager) WaitForCoordinator(ctx context.Context, groupID string) bool {
	for {
		if m.CoordinatorFor(groupID) >= 0 {
			return true
		}
		select {
		case <-ctx.Done():
...
// coordLeaseName builds a valid Kubernetes Lease name for a coordinator group.
func coordLeaseName(groupID string) string {
	safe := nonAlphaNum.ReplaceAllString(strings.ToLower(groupID), "-")
	const prefix = "skafka-coord-"
	if len(safe) > 253-len(prefix) {
		safe = safe[:253-len(prefix)]
	}
	return prefix + safe
}
