...
func (b *Broker) RegisterHandlers(d *protocol.Dispatcher) *protocol.Dispatcher {
	...
	d.Register(3, 1, 12, handlers.NewMetadataHandler(brokerInfo, b.topics, b.leases))
	...
	// Support v0-v4 (v4 same wire format as v3). Key 18 is added to the map manually
	// so the handler can advertise itself.
