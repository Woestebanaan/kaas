...
- `EndTxn` (key 26) fires the offset hook for every recorded group:
  commit materialises the pending offsets into the group's committed
  set (`commit_pending`); abort discards them (`discard_pending`). The
  timeout reaper's abort path fires the same hook, so a reaped
  transaction can't leak half-committed offsets.
